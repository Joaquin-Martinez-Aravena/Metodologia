...
export async function getPagos() {
  const url = `${BASE_URL}/api/pagos/`;
  console.log("🔍 GET", url);

  try {
    const res = await fetch(url);
    const data = await res.json();
    console.log("✅ JSON getPagos:", data);

    if (!res.ok) {
      console.warn("⚠️ Error getPagos:", res.status, data);
      return { __error: true, status: res.status, data };
    }

    return data;
  } catch (err) {
    console.error("💥 Excepción getPagos:", err);
    return { __error: true, status: 0, data: null, error: String(err) };
  }
}

export async function crearPago(body) {
  const url = `${BASE_URL}/api/pagos/realizar`;
  console.log("🔍 POST", url, body);
  ...
  try {
    ...
    const data = await res.json();
    console.log("✅ JSON crearPago:", data);

    if (!res.ok) {
      console.warn("⚠️ Error crearPago:", res.status, data);
      return { __error: true, status: res.status, data };
    }

    return data;
  } catch (err) {
    console.error("💥 Excepción crearPago:", err);
    return { __error: true, status: 0, data: null, error: String(err) };
  }
}